...
class GameConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # echo back the subprotocol (the token) to complete the handshake correctly
        ...
        await self.channel_layer.group_send(
            ...
            {'type': 'game_state', 'fen': game.fen, 'status': game.status}
        )
    ...
    async def handle_move(self, data):
        ...
        await self.channel_layer.group_send(
            self.room_group_name,
            {'type': 'game_state', 'fen': board.fen(), 'status': status, 'winner': winner}
        )
    ...
    def get_or_create_game(self):
        game, _ = OnlineGame.objects.get_or_create(room_code=self.room_code)
        return game

    @database_sync_to_async
    def get_game(self):
        return OnlineGame.objects.get(room_code=self.room_code)
